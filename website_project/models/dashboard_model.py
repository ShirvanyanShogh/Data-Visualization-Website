...
class Dashboard:
    ...
    def percentage_cal(self, energy_sum, energy_avg, session_count,
                       duration_avg, cost_total):
        energy_sum_per = 0
        energy_avg_per = 0
        session_per = 0
        duration_per = 0
        ...
        if self.ok:
            ...
            energy_sum_per = self.percent(float(energy_sum_1),
                                          float(energy_sum))
            energy_avg_per = self.percent(float(energy_avg_1),
                                          float(energy_avg))
            session_per = self.percent(float(session_count_1),
                                       float(session_count))
            duration_per = self.percent(duration_avg_1, duration_avg)
            cost_per = self.percent(float(cost_total_1), float(cost_total))
        return (energy_sum_per, energy_avg_per, session_per, duration_per,
                cost_per)
    ...
    def energy_per_day(self):
        ...
        if self.ok:
            ...
            start_date = f"{int(self.year)}-{self.month_number}-01"
            # ex. 2020-01-01
        ...
        if self.ok:
            ...
            for ind in self.df.index:
                ...
                if time - start_time >= duration:
                    range_str = self.usage_support(time)
                    ranges[range_str] = \
                        ranges[range_str] + duration - accumulated_hours
